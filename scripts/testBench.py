...
def parse_args():
    ...
    requiredNamed.add_argument("-f", required=True, default=None, metavar='FASTA', dest='haplotype_seqs', help="Fasta file containing either the sequences of the true haplotypes or haplotypes sequences (msa) already reported using the same indexing as the reference/consensus sequence")
    requiredNamed.add_argument("-s", required=True, default=None, metavar='CSV', dest='snvs', help="File containing called SNVs")
    parser.add_argument("-m", required=False, default=None, metavar='FASTA', dest='haplotype_master', type=str, help="Fasta file containing the sequence with respect to which SNVs were called")
    parser.add_argument("-d", required=False, default='unif', metavar='str', dest='freq_dstr', type=str, choices=['unif', 'geom'], help="Distribution of haplotype frequencies")
    parser.add_argument("-gr", required=False, default=0.75, metavar='FLOAT', dest='ratio', type=float, help="Sucess probability for the geometric distribution")
    parser.add_argument("-r", required=False, default=None, metavar='chrm:start-end', dest='region', type=str, help="Region in format 'chrm:start-stop', e.g. 'ch3:1000-3000' using 1-based indexing and assuming a closed interval")
    parser.add_argument("-c", required=False, default=False, action='store_true', dest='snv_caller', help="Inidcate if other software different from ShoRAH was used for SNV calling")
    parser.add_argument("-wl", required=False, default=201, metavar='INT', dest='window_len', type=int, help="Window length used by ShoRAH to construct overlapping windows")
    parser.add_argument("-ws", required=False, default=3, metavar='INT', dest='window_shift', type=int, help="Number of window shifts used by ShoRAH to construct overlapping windows")
    parser.add_argument("-ms", required=False, default=False, action='store_true', dest='msa', help="Indicate if the multiple sequence alignment including reference/consensus sequence should be constructed")
    parser.add_argument("-t", required=False, default=False,  action='store_true', dest='output_true', help="Indicate if file containing expected SNVs should be reported. Report using 1-based indexing for the position")
    parser.add_argument("-mafft", required=False, default="mafft", metavar='PATH', dest='mafft', type=str, help="Path to binaries for the multiple sequence aligner MAFFT")
    parser.add_argument("-of", required=False, default='true_snvs.tsv', metavar='OUTPUT', dest='outfile', type=str, help="Output file - file containing expected SNVs")
    parser.add_argument("-od", required=False, default=None, metavar='DIR', dest='outdir', type=str, help="Output directory for intermediate files")
            ...
            if np.sum(idxs) == 1:
                ...
                for a in alphabet:
                    ...
                    if np.sum(idxs_a) > 0:
                        ...
                        haps_true.append(str(hapIDs[0]) if len(idxs_a) == 1 else ','.join((str(x) for x in hapIDs)))
        ...
        with open(snvs_file) as infile:
            for line in infile:
                ...
                if record and record[0] != '#':
                    ...
                    freq = freq.split(";")[0]
                    ref_len = len(row[3]) - 1 # first base corresponds to the locus after which the deletion starts
                    
                    # Check if variants corresponds to a deletion
                    if ref_len >= 1:
                        locus = int(row[1]) # first base corresponds to the locus after which the deletion starts
                        # report deletions per loci w.r.t reference
                        if del_len > 0:
                            idx = locus - loci_inferred_tmp[0]
                            assert idx >= 0
                            loci_inferred = loci_inferred + loci_inferred_tmp[:idx]
                            ref_inferred = ref_inferred + ref_inferred_tmp[:idx]
                            snvs_inferred = snvs_inferred + snvs_inferred_tmp[:idx]
                            freq_inferred = freq_inferred + freq_inferred_tmp[:idx]
...
def get_FN(locus, i, i_max, loci_true, snvs_true, freq_true, FN, missed, haps_true):
    ...
    while loci_true[i] == locus:
        ...
        print("At locus {}, SNV missed {} ({:.6f})".format(loci_true[i] + 1, snvs_true[i], freq_true[i]))
...
def get_FN_TN(locus, i, i_max, loci_true, snvs_true, freq_true, FN, TN, missed, haps_true):
    ...
    if loci_true[i] == locus:
        FN, i, missed = get_FN(locus, i, i_max, loci_true, snvs_true, freq_true, FN, missed, haps_true)
    else:
        if DBG:
...
def get_FP(locus, j, j_max, loci_inferred, snvs_inferred, freq_inferred, FP):
    ...
    while loci_inferred[j] == locus:
        FP += 1
        print("At locus {}, technical error / artefact reported as true variant: {} ({:.6f})".format(loci_inferred[j] + 1, snvs_inferred[j], freq_inferred[j]))
        j += 1
        if j == j_max:
            break
    return(FP, j)
...
def get_FP_TN(locus, j, j_max, loci_inferred, snvs_inferred, freq_inferred, FP, TN):
    ...
    if loci_inferred[j] == locus:
        FP, j = get_FP(locus, j, j_max, loci_inferred, snvs_inferred, freq_inferred, FP)
    else:
        if DBG:
            print("At locus {}, true negative reported".format(locus + 1))
        TN += 1
...
def main():
    ...
    if args.haplotype_master is not None:
        ...
        if args.msa:
            ...
            if haplotype_ref.size != reference_len:
                ...
                for i in range(haplotype_ref.size - reference_len):
                    left = min(reference_len + i - idx_ref, haplotype_master_array[idx_master:].size)
                    idxs = haplotype_ref[idx_ref:(idx_ref + left)] == haplotype_master_array[idx_master:]
                    aux = idxs_ref[idx_ref:(idx_ref + left)][~idxs][0]
                    idx_master = aux - i
                    idx_ref = aux + 1
                    del_idxs[aux] = True
            
                assert np.all(haplotype_ref[~del_idxs] == haplotype_master_array), "After substracting gaps sequences do not agree"
                assert np.all(haplotype_ref[del_idxs] == b'-'), "All substracted loci do not correspond to '-'"
        ...
        if freq_dstr != 'unif':
            ...
            aux = np.repeat(haplotype_seqs_array, np.round(haplotype_freqs * 100).astype(int), axis=0)
    ...
    loci_true, ref_true, snvs_true, freq_true, haps_true = true_snvs(haplotype_master_array, haplotype_seqs_array, num_haplotypes, haplotype_freqs, alphabet)
    
    if args.output_true:
        with open(args.outfile, 'w') as outfile:
            outfile.write('Locus\tRef\tVar\tFreq\tHaplotypes\n')
            for idx in range(len(loci_true)):
               outfile.write('{}\t{}\t{}\t{}\t{}\n'.format(loci_true[idx] + 1, ref_true[idx], snvs_true[idx].decode('utf-8'), freq_true[idx], haps_true[idx])) 
    
    # Compute average frequency for SNVs called using ShoRAH
    loci_inferred, ref_inferred, snvs_inferred, freq_inferred = inferred_snvs(args.snvs) 
    ...
    if args.region is not None:
        ...
        for r in regions:
            ...
            if args.snv_caller:
                ...
                num_windows = np.floor((end_ - (start_ + args.window_len - 1))/(args.window_len // args.window_shift)) + 1
                ...
                end = min(reference_len, start_ + args.window_len - 1 + (num_windows - 2) * (args.window_len // args.window_shift))
            ...
            print("Region with enough support: {:d}-{:d}".format(trunc(start) + 1, trunc(end)))
            ...
            for idx in loci_region:
                ...
                if i == i_max or j == j_max:
                    if i == i_max and j < j_max:
                        ...
                        FP, TN, j = get_FP_TN(idx, j, j_max, loci_inferred, snvs_inferred, freq_inferred, FP, TN)

                    if j == j_max and i < i_max:
                        # There are no more reported SNVs, but some are expected
                        FN, TN, i, missed = get_FN_TN(idx, i, i_max, loci_true, snvs_true, freq_true, FN, TN, missed, haps_true)
                    ...
                    if loci_true[i] == idx and loci_inferred[j] == idx:
                        while loci_true[i] == idx and loci_inferred[j] == idx:
                            if snvs_true[i] == snvs_inferred[j]:
                                ...
                                #print("{}\t{}\t{}".format(loci_true[i] + 1, ref_true[i], snvs_true[i]))
                                if DBG:
                                    print("At locus {}, true positive reported: {} ({:.6f})".format(loci_inferred[j] + 1, snvs_inferred[j], freq_inferred[j]))
                                i += 1
                                j += 1 
                        ...
                        if i < i_max:
                            FN, i, missed = get_FN(idx, i, i_max, loci_true, snvs_true, freq_true, FN, missed, haps_true)
                        if j < j_max:
                            FP, j = get_FP(idx, j, j_max, loci_inferred, snvs_inferred, freq_inferred, FP)
            ...
            if DBG:
                if loci_region[-1] > idx:
                    print("Loci {}-{}, reported as true negative".format(idx + 1, loci_region[-1]))
